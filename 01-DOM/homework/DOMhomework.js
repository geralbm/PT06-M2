...
let spanTitle = document.querySelector ('#createdBy'); //me traigo el span pedido.
//Accedo al texto de span
spanTitle.innerHTML = spanTitle.innerHTML + 'Geraldina'
...
//Para una sola tarea: 
//Se genra la estructura HTML:
  //1-Creo un div
  let toDoShell = document.createElement('div');
  //2- Add una  clase a todoShell
  toDoShell.className = 'toDoShell';
  //3- Creo un span - no tiene valor asigando
  let toDoText = document.createElement ('span');
  //4- Setear el valor a toDoText
...
function displayToDos() {
  ...
  for (let i=0; i<buildedToDos.length;i++ ){
     //elemento html que tiene un div, spam y ese spam tiene la descripcion del ToDo => lo agrego a ToDo container
     toDoContainer.append(buildedToDos[i]);
  }
...
function addToDo() {
  // Tu código acá:
  //Tomo el elemento ToDoInput
 let toDoInput = document.querySelector('#toDoInput');
 //Creo nueva instancia ToDo
